...
gmm_diag_gaussian_log_likelihood = \
    vmap(diag_gaussian_log_likelihood, (None, 0, 0, None))
...
def kl_sample_gmm(key, q_mean_u, q_logvar_u,
  ...
  Args:
    ...
    gmm_p_mean_cxu: np.array 2D array with shape mixture by dist dim,
      means in GMM
...
batch_samples_kl_sample_gmm = \
    vmap(kl_sample_gmm, in_axes=(0, None, None, None, None, None, None))
...
# Returns batch things, so running over
# (keys_bx..., z_mean_bx..., z_logvar_bx...) keys come in with shape (B, S, 2)
batch_kl_sample_gmm = \
    vmap(kl_samples_gmm, in_axes=(0, 0, 0, None, None, None, None))
...
def batch_kl_sample_gmm_pmap_pre(keys_8xbd8xsx2, z_mean_8xbd8xu,
  ...
  Args:
    keys_8xbd8xsx2: numpy array of random.PRNGKey, one for each sample. The
      size is (8, batch_size / 8, S, 2) with S the number of samples. This
      shape is used for setting up a pmap call to 8 devices.
    z_mean_8xbd8xu: mean of gaussian, same shape as keys
    z_logvar_8xbd8xu: logvar of gaussian, same shape as keys
    resps_c: todo
    gmm_z_mean_cxu: todo
    gmm_z_logvar_cxu: todo
    varmin: todo
  Returns:
    KL averaged over S samples.
  """
  ...
  kwargs = {'resps': resps_c, 'gmm_z_mean': gmm_z_mean_cxu,
            'gmm_z_logvar': gmm_z_logvar_cxu, 'varmin': varmin}
  batch_samples_kl_sample_pre_pmap = \
      functools.partial(batch_kl_sample_gmm2, **kwargs)

  pmap_samples = pmap(batch_samples_kl_sample_pre_pmap)

  kl_samples_8xbd8 = pmap_samples(keys_8xbd8xsx2, z_mean_8xbd8xu,
                                  z_logvar_8xbd8xu)
...
def batch_kl_sample_gmm_pmap(keys_bxsx2, z_mean_bxu, z_logvar_bxu, resps_c,
  ...
  # Shard the memory, note the type that goes in vs comes out of the
  # pmap'd lambda.
  ...
  return batch_kl_sample_gmm_pmap_pre(keys_8xbd8xsx2, z_mean_8xbd8xu,
                                      z_logvar_8xbd8xu,
