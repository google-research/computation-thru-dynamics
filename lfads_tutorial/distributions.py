...
def diag_gaussian_sample(key, mean, logvar):
  """x ~ N(mean, exp(logvar))
  ...
  return mean + np.exp(0.5*logvar) * random.normal(key, mean.shape)


def diag_gaussian_log_likelihood(z, mean=0.0, logvar=0.0):
  ...
  return (-0.5 * (logvar + np.log(2*np.pi) +
                  np.square((z-mean)/np.exp(0.5*logvar))))
...
batch_kl_gauss_gauss = vmap(kl_gauss_gauss, in_axes=(0, 0, None))
...
# This function is used in the loss, which is already batch aware. But the
# prior parameters are the same across batch, thus in_axes below.
batch_kl_gauss_ar1 = vmap(kl_gauss_ar1, in_axes=(0, 0, 0, None))
